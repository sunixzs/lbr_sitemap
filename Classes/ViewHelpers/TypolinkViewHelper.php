<?php
namespace LBR\LbrSitemap\ViewHelpers;
/*                                                                        *
...
class TypolinkViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
   ...
   * @return string
   */
  public function render($parameter, $target = '', $extTarget = '', $noCache = 0, $useCacheHash = 1, $additionalParams = array(), $ATagParams = '', $configuration = array()) {
    $typoLinkConf = array(
      'parameter' => $parameter,
    );
    
    if (count($configuraton)) {
      foreach ($configuration as $key => $value) {
        $typoLinkConf[$key] = $value;
      }  
